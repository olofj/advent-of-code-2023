...
fn translate(val: usize, table: &Vec<(usize, usize, usize)>) -> usize {
    // target source size
    for (dst, src, sz) in table {
        if val >= *src && val < (*src + *sz) {
            return *dst + (val - *src);
        }
    }
    val
...
fn mkmap(s: &str) -> Vec<(usize, usize, usize)> {
    ...
    loop {
        match (vals.next(), vals.next(), vals.next()) {
            (Some(dst), Some(src), Some(sz)) => ret.push((dst, src, sz)),
            _ => break,
        }
    }
    ret
...
fn main() {
    ...
    for t in translations {
        let newdata: Vec<_> = data.iter().map(|&d| translate(d, &t)).collect();
        data = newdata;
        println!("data: {:?}", data);
    }
    println!("closest location: {}", data.iter().min().unwrap());
    println!("---");
    println!("Seed 79, soil 81, fertilizer 81, water 81, light 74, temperature 78, humidity 78, location 82");
    println!("Seed 14, soil 14, fertilizer 53, water 49, light 42, temperature 42, humidity 43, location 43");
    println!("Seed 55, soil 57, fertilizer 57, water 53, light 46, temperature 82, humidity 82, location 86");
    println!("Seed 13, soil 13, fertilizer 52, water 41, light 34, temperature 34, humidity 35, location 35");
}
